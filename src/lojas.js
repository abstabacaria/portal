...
// Busca nome + categoria + status da pessoa pelo MAC do aparelho.
// Usado pra saudar clientes conhecidos no portal ("Bem-vindo de volta, X!").
// Retorna null se não reconhecer (aí o portal trata como novo, sem saudação).
async function nomePorAparelho(loja, mac) {
  if (!loja || !loja.id || !mac) return null;
  try {
    // MAC -> telefone (pelo lead mais recente)
    const urlLead = `${SUPABASE_URL}/rest/v1/portal_leads`
      + `?loja_id=eq.${encodeURIComponent(loja.id)}`
      + `&mac=eq.${encodeURIComponent(mac)}`
      + `&select=telefone,nome&order=criado_em.desc&limit=1`;
    const rl = await fetch(urlLead, { headers: H });
    if (!rl.ok) return null;
    const lrows = await rl.json();
    if (!lrows || !lrows[0]) return null;
    const tel = String(lrows[0].telefone || '').replace(/\D/g, '');
    // telefone -> pessoa (nome consolidado + categoria + status)
    if (tel && tel.length >= 10 && tel.length <= 13) {
      const url = `${SUPABASE_URL}/rest/v1/portal_pessoas`
        + `?loja_id=eq.${encodeURIComponent(loja.id)}`
        + `&telefone=eq.${encodeURIComponent(tel)}`
        + `&select=nome,categoria,status,total_visitas&limit=1`;
      const r = await fetch(url, { headers: H });
      if (r.ok) { const rows = await r.json(); if (rows && rows[0]) return rows[0]; }
    }
    // fallback: usa o nome do próprio lead se não achou pessoa consolidada
    if (lrows[0].nome) return { nome: lrows[0].nome, categoria: null, status: null };
  } catch (e) { /* silencioso */ }
  return null;
}
